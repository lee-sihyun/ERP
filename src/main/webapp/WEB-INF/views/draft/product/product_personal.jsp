...
">${product.signerOne}</td>
<td style="
    border: 1px solid;
">${product.signerTwo}</td>
<td style="
    border: 1px solid;
">${product.signerThree}</td>
...
">
 ...
 <c:if 	test="${product.approve==4}">결재승인</c:if>
...
<c:if 	test="${product.approve==4}">결재승인</c:if>
<c:if 	test="${product.approve==3}">결재승인</c:if>
...
">
 ...
 <c:if 	test="${product.approve==4}">결재승인</c:if>
 <c:if 	test="${product.approve==3}">결재승인</c:if>
 <c:if 	test="${product.approve==2}">결재승인</c:if>
<c:if 	test="${product.approve==0}">반려</c:if>
...
금액
		...
		<tr>
			<td style="background: rgb(255, 255, 255); padding: 5px; border: 1px solid black; border-image: none; height: 25px; text-align: left; color: rgb(0, 0, 0); font-size: 14px; font-weight: normal; vertical-align: middle;">
				<span unselectable="on" contenteditable="false" class="comp_wrap" data-cid="23" 
				data-dsl="{{text}}" data-wrapper="" style="width: 100%; font-family: &quot;malgun 
				gothic&quot;, dotum, arial, tahoma; font-size: 11pt; line-height: normal; margin-top: 0px; margin-bottom: 0px;" data-value="">
